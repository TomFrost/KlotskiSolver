...
function hashBoard(pieces, zobMatrix, mirror = false) {
  let hash = 0;
  pieces.forEach(piece => {
    for (let y = piece.y; y < piece.y + piece.h; y++) {
      for (let x = piece.x; x < piece.x + piece.w; x++) {
        if (mirror) {
          hash ^= zobMatrix[y][zobMatrix[0].length - x - 1][PIECE_TYPES[piece.type]];
        } else {
          hash ^= zobMatrix[y][x][PIECE_TYPES[piece.type]];
        }
      }
    }
  });
...
function makeBoard(cols, rows, pieces) {
  const board = new Array(rows).fill(false).map(() => new Array(cols).fill(false));
  pieces.forEach(piece => {
    for (let y = piece.y; y < piece.y + piece.h; y++) {
      for (let x = piece.x; x < piece.x + piece.w; x++) {
        board[y][x] = true;
...
function isBlocked(board, piece, moveVector) {
  ...
  if (moveVector[0] === 1) {
    for (let x = piece.x; x < piece.x + piece.w; x++) {
      if (board[piece.y + piece.h][x]) return true;
    }
  } else if (moveVector[0] === -1) {
    for (let x = piece.x; x < piece.x + piece.w; x++) {
      if (board[piece.y - 1][x]) return true;
    }
  } else if (moveVector[1] === 1) {
    for (let y = piece.y; y < piece.y + piece.h; y++) {
      if (board[y][piece.x + piece.w]) return true;
    }
  } else if (moveVector[1] === -1) {
    for (let y = piece.y; y < piece.y + piece.h; y++) {
      if (board[y][piece.x - 1]) return true;
    }
  }
  return false;
}

function updateHash(zobMatrix, hash, piece, newPiece, mirror = false) {
  let oldHash, newHash;
  if (mirror) {
    oldHash = zobMatrix[piece.y][zobMatrix[0].length - piece.x - 1][PIECE_TYPES[piece.type]];
    newHash = zobMatrix[newPiece.y][zobMatrix[0].length - newPiece.x - 1][PIECE_TYPES[newPiece.type]];
  } else {
    oldHash = zobMatrix[piece.y][piece.x][PIECE_TYPES[piece.type]];
    newHash = zobMatrix[newPiece.y][newPiece.x][PIECE_TYPES[newPiece.type]];
  }
...
(function(){
  window.solve = function(initialState) {
    ...
    const game = {
      ...
      stateQueue: [{
        board: makeBoard(initialState.cols, initialState.rows, initialState.pieces),
        pieces: initialState.pieces,
        lastMove: null, // { id, dir: 0-3 }
        hash: hashBoard(initialState.pieces, zobMatrix),
        mirrorHash: hashBoard(initialState.pieces, zobMatrix, true),
        parentState: null
      }]
    }
